...
class DoubleSnakeIntervention(object):
    '''An intervention where the every other kennel is filled similar to 
    snake but leave an extra space between dogs'''
    ...
    def update(self,simulation):
        ...
        Returns:
            int -- the number of swaps update'''
        
        get_node_state = lambda node: \
            simulation.disease.graph.nodes[node]['data']['occupant']['state']
        get_state_from_id = lambda id: \
            simulation.disease.id_map[id]    
        node_ids = list(sorted([x for x in simulation.disease.graph.nodes]))
        
        # Move out symptomatic and timed nodes
        ...
        while found_gap:
            ...
            for idx, node in enumerate(node_ids):
                if idx == 0 or idx == 1:
                    continue 
                # Check if cell two ahead if unoccupied
                elif get_node_state(node_ids[idx - 2]) == get_state_from_id('E'):
                    # Check is cell ahead if unoccupied
                    if get_node_state(node_ids[idx-1]) == get_state_from_id('E'):
                        # Check if current cell is occupied
                        if get_node_state(node_ids[idx]) != get_state_from_id('E'):               
                            simulation.disease.swap_cells(node, node_ids[idx - 1])
                            self.swap_count += 1
                            found_gap = True  
                            
class TripleSnakeIntervention(object):
    '''An intervention where the every 3rd kennel is filled similar to 
    snake but leave an extra 2 spaces between dogs'''
        ...
        while found_gap:
            ...
            for idx, node in enumerate(node_ids):
                if idx == 0 or idx == 1 or idx ==2:
                    continue 
                # Check if cells three,two,one ahead unoccupied
                elif get_node_state(node_ids[idx - 3]) == get_state_from_id('E') \
                    and get_node_state(node_ids[idx - 2]) == get_state_from_id('E') \
                    and get_node_state(node_ids[idx-1]) == get_state_from_id('E'):
                        # Check if current cell is occupied
                        if get_node_state(node_ids[idx]) != get_state_from_id('E'):               
                            simulation.disease.swap_cells(node, node_ids[idx - 1])
                            self.swap_count += 1
